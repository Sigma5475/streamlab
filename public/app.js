document.addEventListener("DOMContentLoaded", () => {
  ...
  // Affichage du catalogue (films + séries)
  function renderCatalog(data) {
    app.innerHTML = `
      ...
      <div class="catalog">
        ${[...data.series, ...data.movies].map(item => `
          <div class="card" data-id="${item.id}" data-type="${item.seasons ? "series" : "movie"}">
  ...
  function renderSeriesInfo(series) {
      ...
      <div class="series-info">
        <div class="series-header">
          ...
          <div class="series-details">
            ...
            <button id="start-series">Commencer la série</button>
            <button id="back">Retour au catalogue</button>
          </div>
        </div>
        <div class="seasons">
            ...
            <div class="season">
              ...
              <div class="episodes-grid">
                ${season.episodes.map(ep => `
                  <button class="episode-btn" data-src="${ep[2]}">Épisode ${ep[0]} - ${ep[1]}</button>
                `).join("")}
              </div>
            </div>
          `).join("")}
    ...
    // Bouton commencer
    const startBtn = document.getElementById("start-series");
  ...
  function renderMovieInfo(movie) {
    app.innerHTML = `
      <div class="movie-info">
        ...
        <button id="play">Regarder</button>
        <button id="back">Retour au catalogue</button>
        <div id="player-container"></div>
      </div>
    `;
